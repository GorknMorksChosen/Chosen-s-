...
class GameExperienceAnalyzer:
    ...
    def factor_analysis(self, features: list, n_factors: int = 3):
        ...
        def patched_check_array(*args, **kwargs):
            ...
            # 某些旧版 factor_analyzer 可能会传递不支持的参数给新版 sklearn
            # 这里做一个通用的参数过滤，只保留 sklearn.utils.validation.check_array 支持的参数
            # 或者更简单的：如果报错提示不支持某个参数，就在这里拦截
            return original_check_array(*args, **kwargs)
        # 应用补丁
        validation.check_array = patched_check_array
        
        try:
            # 核心修复：直接在调用前确保环境兼容性，或者使用更鲁棒的调用方式
            ...
            # 如果补丁没生效，尝试手动处理特定错误
            if 'force_all_finite' in str(te):
                 raise Exception(f"因子分析环境兼容性错误: {str(te)}。请尝试更新 factor_analyzer 或回退 scikit-learn 版本。")
            raise te
        except Exception as e:
            raise Exception(f"因子分析执行失败: {str(e)}")
        finally:
            # 恢复原始函数
            validation.check_array = original_check_array
    ...
    def regression_analysis(self, features: list, target: str, cluster=None):
        """
        执行多元回归分析
        ...
        df_clean = self.data[selected_all].apply(pd.to_numeric, errors='coerce')
        ...
        if cluster is not None:
            ...
            if '玩家分群' in self.data.columns:
                ...
                df_clean = df_clean.loc[cluster_mask].copy()
        ...
        # 计算VIF
        X_vif_input = sm.add_constant(X_simple)
...
# 1. 数据上传
uploaded_file = st.sidebar.file_uploader("第一步：上传问卷Excel数据", type=["xlsx", "csv"])
...
if uploaded_file:
    # 使用 session_state 存储清洗后的数据，实现跨 Tab 生效
    ...
    if len(selected_features) >= 3:
        ...
        with tab3:
            ...
            try:
                ...
                # 使用分析器执行回归分析
                regression_results = analyzer.regression_analysis(selected_features, target_col, cluster=selected_cluster)
        ...
        with col_sim:
            ...
            try:
                # 重新计算一次整体回归以获取系数
                reg_all = analyzer.regression_analysis(selected_features, target_col)
