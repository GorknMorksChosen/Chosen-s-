...
# --- 1. 配置与初始化 ---
st.set_page_config(page_title="角色战斗标签文本分析工具", layout="wide")
...
class GameAnalysis(BaseModel):
    ...
    key_needs: str = Field(description="玩家最核心的诉求关键词，使用“/”分隔多个短语")
    character_profile: str = Field(description="角色画像描述，包含功能定位、人设、外形和操作特征")
...
def detect_spoiler_columns(target_cols, context_cols, api_key, base_url, model_name):
    ...
    try:
        ...
        tmpl = ChatPromptTemplate.from_template(
            """
你是一名严谨的问卷设计专家，负责审查“背景参考列”是否会对文本填空题造成“剧透”。
...
with st.sidebar:
    ...
    st.info("""
    **如何配置 API:**
    1. **OpenAI**: 保持 Base URL 为默认，填入 `sk-xxxx`
    2. **DeepSeek**: Base URL 填入 `https://api.deepseek.com`
    3. **转发 API**: 填入服务商提供的 Base URL 和 Key
    """)

# --- 3. 核心功能区 ---
st.title("🎮 角色战斗标签文本分析工具")
st.markdown("基于 AI 的游戏调研深度分析引擎，支持跨列背景关联分析。")

uploaded_file = st.file_uploader("上传调研数据 (Excel 或 CSV)", type=["xlsx", "xls", "csv"])
        ...
        if st.button("🚀 开始 AI 深度分析", type="primary", disabled=config_locked):
            ...
            if not st.session_state.api_key:
                ...
                prompt = ChatPromptTemplate.from_template(
                    """
你是一名资深的游戏用户研究员和战斗数值专家。你会收到一个 JSON 数组，其中每个元素代表一名玩家样本，包括背景信息和文本反馈。请对每个样本进行深度分析，并返回一个 JSON 数组作为结果。
...
- key_needs: 玩家最核心的诉求关键词，例如“丝滑手感 / 高机动 / 策略性”，使用“/”分隔多个短语。
- character_profile: 角色画像描述，需综合说明 功能定位（如坦克/刺客）、人设气质、外形特征、操作手感与上手难度。
- emotion_label: 对该角色或体验的整体情绪倾向，建议取值为 "正面" / "中性" / "负面"。
...
4. 数组中的每个对象必须严格包含键: player_segment, key_needs, character_profile, emotion_label, emotion_score。
                    """
                )
                ...
                def process_batch(start_idx, end_idx, api_key, base_url, model_name_local, core_segment_col_local, enable_filter_local, min_length_local, context_cols_local, spoiler_cols_local):
                    ...
                    for _, row in batch_df.iterrows():
                        ...
                        target_text = build_target_string(row, target_cols)
                        raw_values = []
                        for t in target_cols:
                            if t in row:
                                v = sanitize_value(row[t])
                    ...
                    if items:
                        ...
                        try:
                            ...
                            for obj in data_list[: len(items)]:
                                try:
                                    parsed_obj = GameAnalysis.model_validate(obj)
                    ...
                    return start_idx, end_idx, batch_results
                ...
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    ...
                    for future in as_completed(futures):
                        if st.session_state.stop_analysis:
                            ...
                            break
                        s, e, batch_results = future.result()
    ...
    if st.button("生成整体深度报告"):
        try:
            ...
            report_template = ChatPromptTemplate.from_template(
                """
你是一名资深游戏用户研究员，请基于以下已结构化的样本分析结果，撰写一份「深度研究报告」。
...
2. 按玩家分群的横向对比:
   - 对每个玩家分群，概括其核心诉求、偏好的角色特性、情绪特征。
   - 给出类似「约 XX% 的 PVP 玩家提到了‘高位移’或‘灵活机动’」这类量化描述（可以基于提供的占比粗略估计，不要求精确统计）。
3. Top 3 热门角色原型:
   - 从角色画像与情绪表达中，抽取 3 类典型的「角色原型」（例如：高风险高回报刺客 / 团队核心辅助 / 高上限操作型选手等）。
   - 以策划视角描述每个原型：目标玩家是谁、关键体验抓手是什么、有哪些需要注意的设计风险。
4. 情绪与体验风险:
   - 概括高情感强度（高分）背后的正向与负向因素，指出潜在的流失风险点或口碑爆点。
5. 可落地建议:
   - 列出 3-5 条可以直接给到战斗策划 /角色策划/运营同学的行动建议。
...
- 在每一条关键洞察下方，请从 raw_quotes_by_segment 提供的原话池中选取 3-5 条最具代表性的玩家原话，以 Markdown 引用块（> 开头）或列表的形式展示，确保“每一项结论都有原话作证”；
                        ...
                        try:
                            ...
                            if seg_n == 0:
                                ...
                                segment_report_template = ChatPromptTemplate.from_template(
                                    """
你是一名资深游戏用户研究员，请仅针对指定的玩家分群撰写一份「深挖报告」。
...
写作要求:
1. 用 1 段话概括该分群的核心轮廓（他们是谁、玩什么、典型心智）。
2. 从「核心诉求关键词」中抽取 3-5 个最关键的需求，每一个需求都要有准确且客观实际的量化统计（比如有多少人提及，占比多少等），并用策划视角解释它们具体意味着什么。
3. 描述该分群偏好的角色画像：他们更喜欢什么样的职业/操作特征/人设气质/外形风格。
4. 分析该分群的情绪结构：正面驱动力是什么？负面吐槽集中在哪些体验点？
5. 给出 3-5 条专门面向该分群的战斗/角色设计建议。
        ...
        if not base_report:
            ...
            try:
                ...
                ppt_template = ChatPromptTemplate.from_template(
                    """
下面是一份针对角色战斗体验的「深度研究报告」正文。请你将其整理成一份适合策划/制作人快速浏览的 PPT 大纲。
...
4. 要点要尽量短句、可执行，方便直接拷贝到 PPT 中使用。
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        adf.to_excel(writer, index=False, sheet_name="明细+标签")

        report_text = st.session_state.get("deep_report", "")
        if report_text:
            report_lines = str(report_text).splitlines()
        else:
            report_lines = [
                "尚未生成深度研究报告。",
                "",
        ...
        report_df = pd.DataFrame({"整体深度研究报告": report_lines})
        report_df.to_excel(writer, index=False, sheet_name="深度研究报告")

        ppt_text = st.session_state.get("ppt_outline", "")
        if ppt_text:
            ppt_lines = str(ppt_text).splitlines()
        else:
            ppt_lines = [
                "尚未生成 PPT 大纲。",
                "",
        ...
        ppt_df = pd.DataFrame({"PPT 大纲": ppt_lines})
        ppt_df.to_excel(writer, index=False, sheet_name="PPT大纲")
        ...
        if isinstance(segment_reports, dict) and segment_reports:
            for idx_seg, (seg_name, seg_report) in enumerate(
                ...
                seg_lines = str(seg_report).splitlines()
                seg_df = pd.DataFrame({"分群深挖报告": seg_lines})
                seg_df.to_excel(writer, index=False, sheet_name=sheet_name)

    st.download_button(
        label="📥 导出明细、报告与 PPT 大纲 (Excel)",
        data=output.getvalue(),
        file_name="analyzed_game_data_with_report_and_ppt.xlsx",
